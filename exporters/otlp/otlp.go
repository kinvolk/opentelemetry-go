...
func NewExporter(ctx context.Context, cm ConnectionManager, opts ...ExporterOption) (*Exporter, error) {
	exp := NewUnstartedExporter(cm, opts...)
...
func NewUnstartedExporter(cm ConnectionManager, opts ...ExporterOption) *Exporter {
	cfg := config{
...
func (e *Exporter) Export(parent context.Context, cps metricsdk.CheckpointSet) error {
	return e.cm.ExportMetrics(parent, cps, e.cfg.exportKindSelector)
}
...
// transforms and batches trace SpanData into OTLP Trace and transmits them to
// the configured collector.
func (e *Exporter) ExportSpans(ctx context.Context, sds []*tracesdk.SpanData) error {
	return e.cm.ExportTraces(ctx, sds)
}
